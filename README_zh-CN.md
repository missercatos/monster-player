...
<p align="center" style="color:gray;">
  基于 Rust 的 Linux 终端（TUI）音乐播放器，带频谱可视化与塞壬唱片流媒体支持。
</p>
...
这是一个基于 TMPlayer 的 Linux 终端 TUI 音乐播放器，扩展了塞壬唱片（Siren Records）流媒体支持。
支持本地播放、系统播放监控、塞壬唱片流媒体播放，并提供频谱可视化。

<p align="center">
  <img src="overview.png" alt="Monster-Player（TUI Music Player with Streaming）总览">
</p>
...
- 本地音频均衡器支持
- 系统播放监控（MPRIS）
- **塞壬唱片流媒体播放**（专辑浏览器界面、歌曲选择、塞壬唱片 API 流媒体播放）
- 播放列表侧边栏
...
- 播放：rodio（本地）、MPRIS（系统）、ureq（流媒体）
- 可视化：`cava`（外部 bars）
- 流媒体 API：塞壬唱片（Siren Records）API 客户端
...
- Linux：`$XDG_CONFIG_HOME/tmplayer`（通常是 `~/.config/tmplayer`）

也可以用环境变量 `TMPLAYER_ASSET_DIR` 覆盖根目录（该目录下仍使用 `config/` 与 `themes/` 子目录）。
...
1. 环境变量 `TMPLAYER_CAVA`（可为绝对/相对路径）
2. 与程序可执行文件同目录：`./cava`
...
| 按键 | 功能 |
|---|---|
| `N` / `P` 或 `Up` / `Down` | 选择下一个 / 上一个专辑 |
| `Left` / `Right` | 选择当前专辑中的上一首 / 下一首歌曲 |
| `Enter` | 播放选中的歌曲（流媒体） |
| `Q` 或 `Esc` | 关闭专辑浏览器 |
